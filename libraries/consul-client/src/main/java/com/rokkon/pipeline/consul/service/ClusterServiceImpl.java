    ...
    private Uni<ValidationResult> storeClusterMetadata(String clusterName, ClusterMetadata metadata) {
        try {
            ...
            return consulClient.putValue(key, json)
                .onItem().invoke(result -> LOG.debugf("putValue returned: %s", result))
                .onItem().transformToUni(ignored -> {
                    // Try to read it back to verify it was stored
                    return consulClient.getValue(key);
                })
                .map(keyValue -> {
                    if (keyValue != null && keyValue.getValue() != null) {
                        LOG.infof("Successfully verified cluster creation: %s", clusterName);
                        return ValidationResultFactory.success();
                    } else {
                        LOG.warnf("Failed to verify cluster creation - key not found: %s", key);
